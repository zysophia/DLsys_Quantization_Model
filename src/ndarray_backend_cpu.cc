...
void ScalarAdd(const AlignedArray &a, scalar_t val, AlignedArray *out) {
  /**
   * Set entries in out to be the sum of correspondings entry in a plus the
   * scalar val.
   */
  for (size_t i = 0; i < a.size; i++) {
...
/// BEGIN YOUR SOLUTION
enum class _EwiseOp { MUL, DIV, MAX, EQ, GE, LOG, EXP, TANH, SQRT };
...
void Matmul(const AlignedArray &a, const AlignedArray &b, AlignedArray *out,
  ...
  /**
   * Multiply two (compact) matrices into an output (also comapct) matrix.  For
...
inline void AlignedDot(const float *__restrict__ a, const float *__restrict__ b,
   ...
   * the compiler that the input array siwll be aligned to the appropriate
   * blocks in memory, which also helps the compiler vectorize the code.
   *
   * Args:
