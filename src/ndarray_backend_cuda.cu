...
__global__ void CompactKernel(const scalar_t *a, scalar_t *out, size_t size,
  ...
  for (size_t i = 0; i < dim; ++i) {
    int cur = gid / base;
    cur %= shape.data[dim - i - 1];
    offset += cur * strides.data[dim - i - 1];
    base *= shape.data[dim - i - 1];
  }
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray *out,
  ...
  ScalarSetitemKernel<<<dim.grid, dim.block>>>(
      out->size, val, out->ptr, VecToCuda(shape), VecToCuda(strides), offset,
      _dim);
  /// END YOUR SOLUTION
}
...
/// BEGIN YOUR SOLUTION
enum class _EwiseOp { MUL, DIV, MAX, EQ, GE, LOG, EXP, TANH };
enum class _ScalarOp { MUL, DIV, POWER, MAX, EQ, GE };

__global__ void _EwiseOpKernel(const scalar_t *a, const scalar_t *b,
                               scalar_t *out, size_t size, _EwiseOp op) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (op == _EwiseOp::MUL) {
    if (gid < size)
      out[gid] = a[gid] * b[gid];
...
void EwiseOp(const CudaArray &a, const CudaArray &b, CudaArray *out) {
  CudaDims dim = CudaOneDim(out->size);
  _EwiseOpKernel<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr, out->size,
                                          op);
}

__global__ void _EwiseFuncKernel(const scalar_t *a, scalar_t *out, size_t size,
                                 _EwiseOp op) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (op == _EwiseOp::LOG) {
    if (gid < size)
...
template <_EwiseOp op> void EwiseFunc(const CudaArray &a, CudaArray *out) {
  CudaDims dim = CudaOneDim(out->size);
  _EwiseFuncKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size, op);
}

__global__ void _ScalarOpKernel(const scalar_t *a, scalar_t val, scalar_t *out,
                                size_t size, _ScalarOp op) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (op == _ScalarOp::MUL) {
    if (gid < size)
      out[gid] = a[gid] * val;
  ...
  if (op == _ScalarOp::POWER) {
    if (gid < size)
      out[gid] = pow(a[gid], val);
  }
  if (op == _ScalarOp::EQ) {
    if (gid < size)
      out[gid] = (size_t)(a[gid] == val);
  }
  if (op == _ScalarOp::GE) {
    if (gid < size)
      out[gid] = (size_t)(a[gid] >= val);
...
void ScalarOp(const CudaArray &a, scalar_t val, CudaArray *out) {
  CudaDims dim = CudaOneDim(out->size);
  _ScalarOpKernel<<<dim.grid, dim.block>>>(a.ptr, val, out->ptr, out->size, op);
}
/// END YOUR SOLUTION
...
__global__ void _ReduceOpKernel(const scalar_t *a, scalar_t *out,
                                size_t reduce_size, size_t size, _ReduceOp op) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (op == _ReduceOp::MAX) {
    scalar_t val = a[gid * reduce_size];
    for (size_t j = 0; j < reduce_size; ++j) {
      val = fmaxf(val, a[gid * reduce_size + j]);
    }
    if (gid < size)
      out[gid] = val;
  }
...
void ReduceMax(const CudaArray &a, CudaArray *out, size_t reduce_size) {
  ...
  /// BEGIN YOUR SOLUTION
  CudaDims dim = CudaOneDim(out->size);
  _ReduceOpKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, reduce_size,
                                           out->size, _ReduceOp::MAX);
  /// END YOUR SOLUTION
}
